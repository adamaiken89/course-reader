...
describe('highlightsStore', () => {
  ...
  test('getForModule returns empty array for unknown module', () => {
    expect(useHighlightsStore.getState().getForModule('course1', 1)).toEqual([]);
  });
  ...
  test('load fetches highlights and stores by module key', async () => {
    ...
    useHighlightsStore.getState().load('course1', 1);
    await new Promise((resolve) => setTimeout(resolve, 0));
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(useHighlightsStore.getState().getForModule('course1', 1)).toEqual(mockHighlights);
  });
  ...
  test('add posts highlight and appends to byModule', async () => {
    ...
    await useHighlightsStore.getState().add('course1', 1, 'text', 'green', 0, 4);
    const highlights = useHighlightsStore.getState().getForModule('course1', 1);
